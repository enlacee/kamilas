...
class Apps extends Config {
    ...
    public function getPostBy($post_type = '', $status = 1, $limit = '', $field = array())
    {
        $this->acentosQuery();
        $sql = "SELECT ";
        ...
        if (!empty($post_type)) {
            $sql .= "FROM posts WHERE post_type = {$post_type} AND status = {$status} ";
        } else {
           $sql .= "FROM posts WHERE status = {$status} "; 
        }
